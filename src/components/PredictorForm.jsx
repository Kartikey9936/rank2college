...
const SelectField = ({ id, label, name, value, onChange, children }) => (
  <div className="flex flex-col gap-2">
    <label htmlFor={id} className="text-slate-500 dark:text-white/70 text-xs font-semibold uppercase tracking-wider">{label}</label>
    <div className="relative">
      <select id={id} name={name} value={value} onChange={onChange}
        className="input-field w-full px-4 py-3.5 rounded-xl appearance-none pr-10 font-medium text-sm cursor-pointer">
        {children}
      </select>
...
export default function PredictorForm({ filters, setFilters, onPredict, isLoading }) {
  ...
  return (
    <div className="glass rounded-3xl p-6 md:p-8 lg:p-10">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h2 className="text-slate-800 dark:text-white font-bold text-lg">Your Profile</h2>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="flex flex-col gap-2 md:col-span-2 lg:col-span-1">
          <label htmlFor="rank" className="text-slate-500 dark:text-white/70 text-xs font-semibold uppercase tracking-wider">Your JEE Rank</label>
          <input type="number" id="rank" name="rank"
            className="input-field px-4 py-3.5 rounded-xl font-bold text-xl text-center"
      ...
      <div className="border-t border-slate-200 dark:border-white/10 my-7" />
      ...
      <button className="btn-primary w-full mt-8 text-white font-bold text-lg py-4 rounded-2xl
